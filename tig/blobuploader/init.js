...
(function () {
        ...
        config = {
            ...
            on: {
                ...
                instanceReady: function (event) {
                    ...
                    event.editor.document.on('drop', async function (ev) {
                        console.group('instanceReady Drop event:', ev.data.$.dataTransfer.files);
                        ev.data.preventDefault(true);
                
                        try {
                            // This method is defined by blobuploader
                            await window.uploadFiles(ev.data.$.dataTransfer.files);
                ...
                dragstart: function (evt) {
                    // Disable drag and drop within editor. It conflicts with blobuploader
                    console.log('Drag started:', evt.data.$);
                    evt.cancel();
                },
                drop: async function (evt) {
                    console.log('CKEditor drop event:', evt.data);
                    evt.cancel();

                    // This method is defined by blobuploader
                    try {
                        await window.uploadFiles(evt.data.$.dataTransfer.files);
                        console.log('Files uploaded successfully.');
                    } catch (error) {
                        console.error('Error uploading files:', error);
                    }
    ...
    //config.extraPlugins.push('codeSnippets');
